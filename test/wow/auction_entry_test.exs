defmodule Wow.AuctionEntryTest do
  ...
  describe "auction entry" do
    ...
    @valid_attrs %{auc_id: 1, bid: 10, item: 11, owner: "foo", owner_realm: "medivh", buyout: 20,
      quantity: 1, time_left: "VERY LONG", rand: 0, seed: 0, context: 0,
      dump_timestamp: DateTime.from_unix!(1542513262) |> DateTime.truncate(:second)}
    @invalid_attrs %{auc_id: nil, bid: nil, item: nil, owner: nil, owner_realm: nil, buyout: nil,
      quantity: nil, time_left: nil, rand: nil, seed: nil, context: nil, dump_timestamp: nil}
    ...
    test "from_raw/2 builds an entry" do
      assert entry = AuctionEntry.from_raw(@valid_raw, 1542513262)
      assert entry.valid?
    end

    test "from_raw/2 validates values" do
      assert entry = AuctionEntry.from_raw(Map.delete(@valid_raw, "auc"), 1542513262)
      refute entry.valid?
    end
